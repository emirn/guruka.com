---
title: "Sequence Recall - Brain Training Game"
...
<p class="sr-subtitle">Train your working memory</p>
<ul class="sr-instructions-list">
<li data-num="1">Watch the sequence of colors shown one at a time</li>
<li data-num="2">Memorize the order carefully</li>
<li data-num="3">Recall the colors in the exact same order</li>
<li data-num="4">Progress through 20 levels of increasing difficulty</li>
</ul>
...
<div class="sr-progress-label" id="sr-progress-label">Round 1 of 3</div>
<div class="sr-progress-bar">
<div class="sr-progress-fill" id="sr-progress-fill" style="width:0%"></div>
</div>
</div>
<button class="gk-finish-btn" id="sr-finish-btn">Finish Game</button>
</div>

<div id="sr-complete" class="sr-screen">
<div class="sr-card sr-complete">
<div class="sr-complete-icon">&#x1F3C6;</div>
<h2 class="sr-complete-title">Game Complete!</h2>
<div id="sr-new-best-badge" class="sr-new-best" style="display:none;">New Personal Best!</div>
...
<button class="sr-btn-primary" id="sr-play-again-btn">Play Again</button>
...
var COLORS = [
{ name: 'Red', hsl: 'hsl(0,84%,60%)' },
{ name: 'Blue', hsl: 'hsl(217,91%,60%)' },
{ name: 'Green', hsl: 'hsl(142,76%,36%)' },
{ name: 'Yellow', hsl: 'hsl(48,96%,53%)' },
{ name: 'Purple', hsl: 'hsl(280,68%,55%)' },
...
$('sr-progress-label').textContent = 'Round ' + (state.round + 1) + ' of ' + ROUNDS_PER_LEVEL;
...
$('sr-phase-label').textContent = 'Memorize the colors';
state.currentShowIndex = 0;
showNextColor();
}
...
$('sr-phase-label').textContent = 'Memorize the positions';
state.currentShowIndex = 0;
showNextPosition();
}
...
$('sr-phase-label').textContent = 'Quick math!';
...
html += '</div>';
html += '<div class="sr-math-hint">Solve to continue</div>';
html += '</div>';
...
$('sr-phase-label').textContent = 'Recall the color sequence';
state.selectedColors = [];
...
html += '<span class="sr-selected-placeholder">Tap colors in order (' + needed + ' needed)</span>';
} else {
...
html += '<button class="sr-undo-btn" id="sr-undo-btn">Undo</button>';
html += '<button class="sr-submit-btn" id="sr-submit-colors-btn"' + (state.selectedColors.length < needed ? ' disabled' : '') + '>Submit</button>';
html += '</div>';
...
$('sr-phase-label').textContent = 'Recall the positions in order';
state.selectedPositions = [];
state.phaseStartTime = Date.now();
...
html += '<span class="sr-selected-placeholder">Tap grid positions in order (' + needed + ' needed)</span>';
} else {
