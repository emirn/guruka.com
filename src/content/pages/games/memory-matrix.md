---
title: "Memory Matrix - Brain Training Game"
...
<h3>How to Play:</h3>
<ol>
<li><span class="mm-step-num">1</span><span>Watch the pattern -- cells will highlight one by one</span></li>
<li><span class="mm-step-num">2</span><span>Memorize which cells light up</span></li>
<li><span class="mm-step-num">3</span><span>Tap the same cells from memory</span></li>
<li><span class="mm-step-num">4</span><span>From level 8, order matters! Tap in the right sequence</span></li>
</ol>
</div>
...
<div class="mm-header-item">
<div class="mm-header-label">Streak</div>
...
<div class="mm-progress-text" id="mm-progress-text">Round 1 of 3</div>
</div>
<div class="mm-actions" id="mm-actions"></div>
<button class="gk-finish-btn" id="mm-finish-btn">Finish Game</button>
</div>
...
<h1 class="mm-title">Game Complete!</h1>
<div class="mm-score-display" id="mm-final-score">0</div>
<div id="mm-new-best-badge" class="mm-new-best" style="display:none;">New Personal Best!</div>
...
<div class="mm-stat-label">Accuracy</div>
</div>
<div class="mm-stat-card">
<div class="mm-stat-value" id="mm-final-streak">0</div>
<div class="mm-stat-label">Best Streak</div>
</div>
</div>
<div class="mm-complete-actions">
<button class="mm-btn mm-btn-primary" onclick="mmPlayAgain()">Play Again</button>
...
function updateInstructionsBest() {
  ...
  if (best > 0 && banner) {
    ...
    banner.innerHTML = 'Personal Best: <strong>' + best.toLocaleString() + '</strong>';
  }
}
...
function updatePhase(config) {
  ...
  if (state.phase === 'showing') {
    phaseEl.innerHTML = '<span class="mm-phase-memorize">Memorize the pattern...</span>';
  } else if (state.phase === 'recall') {
    var orderNote = config.orderMatters ? ' (order matters!)' : '';
    phaseEl.innerHTML = '<span class="mm-phase-recall">Tap the cells!' + orderNote + ' &mdash; ' + state.selectedCells.length + '/' + state.pattern.length + ' selected</span>';
  }
}
...
function updateActions(config) {
  ...
  actionsEl.innerHTML =
    '<button class="mm-btn mm-btn-outline" onclick="mmClearSelection()"' + clearDisabled + '>Clear</button>' +
    '<button class="mm-btn mm-btn-submit" onclick="mmSubmitAnswer()"' + submitDisabled + '>Submit (' + state.selectedCells.length + '/' + state.pattern.length + ')</button>';
}
...
function updateProgressBar(config) {
  ...
  document.getElementById('mm-progress-text').textContent =
    'Round ' + (state.roundsInLevel + 1) + ' of ' + config.roundsPerLevel;
}
...
function showFeedback(correct, config) {
  ...
  if (correct) {
    ...
    phaseEl.innerHTML = '<span class="mm-phase-wrong">Not quite...</span>';
  }
