---
title: "Sequence Recall - Игра для мозга"
description: "Запоминайте последовательность цветов и тренируйте рабочую память. Наблюдайте за цветами и воспроизведите их в правильном порядке!"
full_width: true
language: "ru"
---
...
<p class="sr-subtitle">Тренируйте рабочую память</p>
<ul class="sr-instructions-list">
<li data-num="1">Наблюдайте за цветами, которые появляются по очереди</li>
<li data-num="2">Внимательно запоминайте последовательность</li>
<li data-num="3">Воспроизведите цвета в точно таком же порядке</li>
<li data-num="4">Пройдите 20 уровней возрастающей сложности</li>
</ul>
<div class="sr-best-score" id="sr-best-display">
Лучший результат: <strong id="sr-best-value">0</strong> очков
</div>
<button class="sr-btn-primary" id="sr-start-btn">Начать игру</button>
...
<span class="sr-header-label">Очки</span>
...
<div class="sr-header-item">
<span class="sr-header-label">Серия</span>
...
<div class="sr-progress-label" id="sr-progress-label">Раунд 1 / 3</div>
<div class="sr-progress-bar">
<div class="sr-progress-fill" id="sr-progress-fill" style="width:0%"></div>
</div>
</div>
<button class="gk-finish-btn" id="sr-finish-btn">Завершить игру</button>
</div>

<div id="sr-complete" class="sr-screen">
<div class="sr-card sr-complete">
<div class="sr-complete-icon">&#x1F3C6;</div>
<h2 class="sr-complete-title">Игра окончена!</h2>
<div id="sr-new-best-badge" class="sr-new-best" style="display:none;">Новый личный рекорд!</div>
...
<span class="sr-stat-label">Очки</span>
</div>
<div class="sr-stat-box">
...
<span class="sr-stat-label">Точность</span>
</div>
</div>
<div class="sr-complete-links">
<button class="sr-btn-primary" id="sr-play-again-btn">Играть снова</button>
<a href="/ru/games/" class="sr-btn-secondary">Все игры</a>
</div>
<div class="gk-share-section">

<button class="gk-share-btn" id="sr-share-btn">&#128279; Поделиться результатом</button>
<div class="gk-share-copied" id="sr-share-copied" style="display:none;">Ссылка скопирована!</div>
</div>
</div>
</div>
...
var COLORS = [
{ name: 'Красный', hsl: 'hsl(0,84%,60%)' },
{ name: 'Синий', hsl: 'hsl(217,91%,60%)' },
{ name: 'Зелёный', hsl: 'hsl(142,76%,36%)' },
{ name: 'Жёлтый', hsl: 'hsl(48,96%,53%)' },
{ name: 'Фиолет.', hsl: 'hsl(280,68%,55%)' },
{ name: 'Оранж.', hsl: 'hsl(25,95%,53%)' }
];
...
$('sr-progress-label').textContent = 'Раунд ' + (state.round + 1) + ' / ' + ROUNDS_PER_LEVEL;
var pct = (state.round / ROUNDS_PER_LEVEL) * 100;
$('sr-progress-fill').style.width = pct + '%';
}

var SR_WIZARD_STEPS = [
{icon: '\uD83C\uDFA8', title: 'Наблюдайте за цветами', desc: 'Цвета будут показаны по очереди. Запомните их порядок!'},
{icon: '\uD83E\uDDE0', title: 'Воспроизведите по памяти', desc: 'Нажимайте на цвета в том же порядке, в котором они были показаны.'},
{icon: '\uD83D\uDE80', title: 'Готовы? Начинаем!', desc: 'Последовательности будут становиться длиннее. Сосредоточьтесь!', final: true}
];
var srWizardStep = 0;
...
var btnLabel = s.final ? 'Начать игру' : 'Далее';
...
$('sr-phase-label').textContent = 'Запомните цвета';
state.currentShowIndex = 0;
showNextColor();
}
...
$('sr-phase-label').textContent = 'Запомните позиции';
state.currentShowIndex = 0;
showNextPosition();
}
...
$('sr-phase-label').textContent = 'Математическое задание!';
...
html += '<button class="sr-math-submit" id="sr-math-btn">OK</button>';
html += '</div>';
html += '<div class="sr-math-hint">Решите, чтобы продолжить</div>';
html += '</div>';
...
$('sr-phase-label').textContent = 'Воспроизведите последовательность цветов';
state.selectedColors = [];
...
html += '<span class="sr-selected-placeholder">Нажимайте на цвета по порядку (' + needed + ' шт.)</span>';
} else {
...
html += '<button class="sr-undo-btn" id="sr-undo-btn">Отменить</button>';
html += '<button class="sr-submit-btn" id="sr-submit-colors-btn"' + (state.selectedColors.length < needed ? ' disabled' : '') + '>Подтвердить</button>';
html += '</div>';
...
$('sr-phase-label').textContent = 'Воспроизведите позиции по порядку';
state.selectedPositions = [];
state.phaseStartTime = Date.now();
...
html += '<span class="sr-selected-placeholder">Нажимайте на ячейки по порядку (' + needed + ' шт.)</span>';
} else {
...
html += '<button class="sr-undo-btn" id="sr-undo-pos-btn">Отменить</button>';
html += '<button class="sr-submit-btn" id="sr-submit-pos-btn"' + (state.selectedPositions.length < needed ? ' disabled' : '') + '>Подтвердить</button>';
html += '</div>';
...
if (correct) {
html += '<div class="sr-feedback-correct">Правильно!</div>';
html += '<div class="sr-feedback-points">+' + points + ' очков</div>';
} else {
html += '<div class="sr-feedback-wrong">Неправильно</div>';
html += '<div class="sr-feedback-points">Серия сброшена</div>';
}
html += '</div></div>';
...
srShowConfetti();
iconEl.textContent = '\uD83C\uDF89';
titleEl.textContent = 'Великолепно!';
badge.className = 'sr-new-best-enhanced';
badge.innerHTML = 'uD83CuDF1F Великолепно! Новый рекорд!';
...
GK.shareResult(state.score, 'Sequence Recall', '/ru/games/sequence-recall/', 'sr-share-copied');
});
