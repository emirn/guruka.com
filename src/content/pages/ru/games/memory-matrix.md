---
title: "Memory Matrix - Игра для мозга"
description: "Тренировка пространственной памяти: запоминайте и воспроизводите паттерны на сетке. Наблюдайте за паттерном и нажимайте на ячейки по памяти!"
full_width: true
language: "ru"
---
...
<p class="mm-subtitle">Проверьте свою пространственную память</p>
<div class="mm-how-to">
<h3>Как играть:</h3>
<ol>
<li><span class="mm-step-num">1</span><span>Наблюдайте за паттерном -- ячейки будут подсвечиваться по очереди</span></li>
<li><span class="mm-step-num">2</span><span>Запомните, какие ячейки подсветились</span></li>
<li><span class="mm-step-num">3</span><span>Нажмите на те же ячейки по памяти</span></li>
<li><span class="mm-step-num">4</span><span>С уровня 8 важен порядок! Нажимайте ячейки в правильной последовательности</span></li>
</ol>
</div>
<div id="mm-instr-best" class="mm-best-banner" style="display:none;"></div>
<button class="mm-btn mm-btn-primary" onclick="mmShowWizard()">Начать игру</button>
...
<div class="mm-header-label">Очки</div>
<div class="mm-header-value" id="mm-score">0</div>
</div>
<div class="mm-header-item">
<div class="mm-header-label">Level</div>
<div class="mm-header-value" id="mm-level">1</div>
</div>
<div class="mm-header-item">
<div class="mm-header-label">Серия</div>
...
<div class="mm-progress-text" id="mm-progress-text">Раунд 1 / 3</div>
</div>
<div class="mm-actions" id="mm-actions"></div>
<button class="gk-finish-btn" id="mm-finish-btn">Завершить игру</button>
</div>
...
<h1 class="mm-title">Игра окончена!</h1>
<div class="mm-score-display" id="mm-final-score">0</div>
<div id="mm-new-best-badge" class="mm-new-best" style="display:none;">Новый личный рекорд!</div>
...
<div class="mm-stat-label">Level</div>
</div>
<div class="mm-stat-card">
<div class="mm-stat-value" id="mm-final-accuracy">0%</div>
<div class="mm-stat-label">Точность</div>
</div>
<div class="mm-stat-card">
<div class="mm-stat-value" id="mm-final-streak">0</div>
<div class="mm-stat-label">Лучшая серия</div>
</div>
</div>
<div class="mm-complete-actions">
<button class="mm-btn mm-btn-primary" onclick="mmPlayAgain()">Играть снова</button>
<a href="/ru/games/" class="mm-back-link">Вернуться ко всем играм</a>
</div>
<div class="gk-share-section">

<button class="gk-share-btn" id="mm-share-btn">&#128279; Поделиться результатом</button>
<div class="gk-share-copied" id="mm-share-copied" style="display:none;">Ссылка скопирована!</div>
</div>
</div>
</div>
...
function updateInstructionsBest() {
  ...
  if (best > 0 && banner) {
    ...
    banner.innerHTML = 'Лучший результат: <strong>' + best.toLocaleString() + '</strong> очков';
  }
}

/* ---- Wizard ---- */
var MM_WIZARD_STEPS = [
{icon: '\uD83D\uDCA1', title: 'Наблюдайте за ячейками', desc: 'Ячейки будут подсвечиваться по очереди. Смотрите внимательно!'},
{icon: '\uD83D\uDC46', title: 'Нажимайте по памяти', desc: 'После исчезновения паттерна воспроизведите его по памяти.'},
{icon: '\uD83D\uDE80', title: 'Готовы? Начинаем!', desc: 'С повышением уровня паттерны становятся сложнее!', final: true}
];
var mmWizardStep = 0;
...
function updatePhase(config) {
  ...
  if (state.phase === 'showing') {
    phaseEl.innerHTML = '<span class="mm-phase-memorize">Запомните паттерн...</span>';
  } else if (state.phase === 'recall') {
    var orderNote = config.orderMatters ? ' (порядок важен!)' : '';
    phaseEl.innerHTML = '<span class="mm-phase-recall">Ваш ход!' + orderNote + ' &mdash; ' + state.selectedCells.length + '/' + state.pattern.length + ' выбрано</span>';
  }
}
...
function updateActions(config) {
  ...
  actionsEl.innerHTML =
    '<button class="mm-btn mm-btn-outline" onclick="mmClearSelection()"' + clearDisabled + '>Сбросить</button>' +
    '<button class="mm-btn mm-btn-submit" onclick="mmSubmitAnswer()"' + submitDisabled + '>Отправить (' + state.selectedCells.length + '/' + state.pattern.length + ')</button>';
}
...
function updateProgressBar(config) {
  ...
  document.getElementById('mm-progress-text').textContent =
    'Раунд ' + (state.roundsInLevel + 1) + ' / ' + config.roundsPerLevel;
}
...
function showFeedback(correct, config) {
  ...
  var phaseEl = document.getElementById('mm-phase');
  if (correct) {
    phaseEl.innerHTML = '<span class="mm-phase-correct">Правильно!</span>';
  } else {
    phaseEl.innerHTML = '<span class="mm-phase-wrong">Неверно...</span>';
  }
...
function endGame() {
  ...
  if (isNewBest) {
    ...
    bestBadge.innerHTML = 'uD83CuDF1F Великолепно! Новый рекорд!';
    bestBadge.style.display = 'inline-block';
  } else {
    titleEl.textContent = 'Отличная работа!';
...
document.getElementById('mm-share-btn').addEventListener('click', function() {
  GK.shareResult(state.score, 'Memory Matrix', '/ru/games/memory-matrix/', 'mm-share-copied');
});
})();
</script>
