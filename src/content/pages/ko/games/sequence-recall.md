---
title: "Sequence Recall - 두뇌 훈련 게임"
description: "색의 순서를 기억하여 작업 기억력을 단련하세요. 색을 관찰하고 올바른 순서로 떠올려 보세요!"
full_width: true
language: "ko"
---
<style>
...
<span class="sr-header-label">점수</span>
...
<div class="sr-header-item">
<span class="sr-header-label">연속</span>
...
<div class="sr-progress-label" id="sr-progress-label">라운드 1 / 3</div>
<div class="sr-progress-bar">
<div class="sr-progress-fill" id="sr-progress-fill" style="width:0%"></div>
</div>
</div>
<button class="gk-finish-btn" id="sr-finish-btn">게임 종료</button>
</div>

<div id="sr-complete" class="sr-screen">
<div class="sr-card sr-complete">
<div class="sr-complete-icon">&#x1F3C6;</div>
<h2 class="sr-complete-title">게임 종료!</h2>
<div id="sr-new-best-badge" class="sr-new-best" style="display:none;">새로운 개인 최고!</div>
...
<span class="sr-stat-label">점수</span>
</div>
<div class="sr-stat-box">
...
<span class="sr-stat-label">정확도</span>
</div>
</div>
<div class="sr-complete-links">
<button class="sr-btn-primary" id="sr-play-again-btn">다시하기</button>
<a href="/ko/games/" class="sr-btn-secondary">모든 게임</a>
</div>
<div class="gk-share-section">

<button class="gk-share-btn" id="sr-share-btn">&#128279; 결과 공유</button>
<div class="gk-share-copied" id="sr-share-copied" style="display:none;">링크가 복사되었습니다!</div>
</div>
</div>
</div>
...
var COLORS = [
{ name: '빨강', hsl: 'hsl(0,84%,60%)' },
{ name: '파랑', hsl: 'hsl(217,91%,60%)' },
{ name: '초록', hsl: 'hsl(142,76%,36%)' },
{ name: '노랑', hsl: 'hsl(48,96%,53%)' },
{ name: '보라', hsl: 'hsl(280,68%,55%)' },
{ name: '주황', hsl: 'hsl(25,95%,53%)' }
];
...
$('sr-progress-label').textContent = '라운드 ' + (state.round + 1) + ' / ' + ROUNDS_PER_LEVEL;
var pct = (state.round / ROUNDS_PER_LEVEL) * 100;
$('sr-progress-fill').style.width = pct + '%';
}

var SR_WIZARD_STEPS = [
{icon: '\uD83C\uDFA8', title: '색의 배열을 관찰하세요', desc: '색이 순서대로 표시됩니다. 순서를 기억하세요!'},
{icon: '\uD83E\uDDE0', title: '순서대로 떠올리세요', desc: '본 순서대로 색을 탭하세요.'},
{icon: '\uD83D\uDE80', title: '준비됐나요? 시작!', desc: '배열이 길어집니다. 집중하세요!', final: true}
];
var srWizardStep = 0;
...
var btnLabel = s.final ? '플레이 시작' : '다음';
...
$('sr-phase-label').textContent = '색을 기억하세요';
state.currentShowIndex = 0;
showNextColor();
}
...
$('sr-phase-label').textContent = '위치를 기억하세요';
state.currentShowIndex = 0;
showNextPosition();
}
...
$('sr-phase-label').textContent = '계산 챌린지!';
...
html += '</div>';
html += '<div class="sr-math-hint">풀고 진행</div>';
html += '</div>';
...
$('sr-phase-label').textContent = '색의 배열을 떠올리세요';
state.selectedColors = [];
...
html += '<span class="sr-selected-placeholder">색을 순서대로 탭 (' + needed + ' 개 필요)</span>';
} else {
...
html += '<button class="sr-undo-btn" id="sr-undo-btn">되돌리기</button>';
html += '<button class="sr-submit-btn" id="sr-submit-colors-btn"' + (state.selectedColors.length < needed ? ' disabled' : '') + '>확인</button>';
html += '</div>';
...
$('sr-phase-label').textContent = '순서대로 위치를 떠올리세요';
state.selectedPositions = [];
state.phaseStartTime = Date.now();
...
html += '<span class="sr-selected-placeholder">그리드 위치를 순서대로 탭 (' + needed + ' 개 필요)</span>';
} else {
...
html += '<button class="sr-undo-btn" id="sr-undo-pos-btn">되돌리기</button>';
html += '<button class="sr-submit-btn" id="sr-submit-pos-btn"' + (state.selectedPositions.length < needed ? ' disabled' : '') + '>확인</button>';
html += '</div>';
...
if (correct) {
html += '<div class="sr-feedback-correct">정답!</div>';
html += '<div class="sr-feedback-points">+' + points + ' 포인트</div>';
} else {
html += '<div class="sr-feedback-wrong">오답</div>';
html += '<div class="sr-feedback-points">연속 초기화</div>';
}
html += '</div></div>';
...
srShowConfetti();
iconEl.textContent = '\uD83C\uDF89';
titleEl.textContent = '대단해요!';
badge.className = 'sr-new-best-enhanced';
badge.innerHTML = 'uD83CuDF1F 대단해요!신기록!';
...
GK.shareResult(state.score, 'Sequence Recall', '/ko/games/sequence-recall/', 'sr-share-copied');
});
