---
title: "Memory Matrix - 두뇌 훈련 게임"
description: "그리드 패턴을 기억하고 재현하는 공간 기억력 트레이닝. 패턴을 관찰하고 기억에서 셀을 탭하세요!"
full_width: true
language: "ko"
---
<style>
...
<div class="mm-header-label">점수</div>
...
<div class="mm-header-item">
<div class="mm-header-label">연속</div>
...
<div class="mm-progress-text" id="mm-progress-text">라운드 1 / 3</div>
</div>
<div class="mm-actions" id="mm-actions"></div>
<button class="gk-finish-btn" id="mm-finish-btn">게임 종료</button>
</div>
...
<div class="mm-stat-label">정확도</div>
</div>
<div class="mm-stat-card">
<div class="mm-stat-value" id="mm-final-streak">0</div>
<div class="mm-stat-label">최고 연속</div>
</div>
</div>
<div class="mm-complete-actions">
<button class="mm-btn mm-btn-primary" onclick="mmPlayAgain()">다시하기</button>
<a href="/ko/games/" class="mm-back-link">모든 게임으로 돌아가기</a>
</div>
<div class="gk-share-section">

<button class="gk-share-btn" id="mm-share-btn">&#128279; 결과 공유</button>
<div class="gk-share-copied" id="mm-share-copied" style="display:none;">링크가 복사되었습니다!</div>
</div>
</div>
</div>
...
/* ---- Wizard ---- */
var MM_WIZARD_STEPS = [
{icon: '\uD83D\uDCA1', title: '빛나는 셀을 관찰하세요', desc: '셀이 순서대로 빛납니다. 잘 보세요!'},
{icon: '\uD83D\uDC46', title: '기억에서 탭하세요', desc: '패턴이 사라진 후 기억에서 재현하세요.'},
{icon: '\uD83D\uDE80', title: '준비됐나요? 시작!', desc: '레벨이 올라가면 패턴이 어려워집니다!', final: true}
];
var mmWizardStep = 0;
...
function updatePhase(config) {
  ...
  if (state.phase === 'showing') {
    phaseEl.innerHTML = '<span class="mm-phase-memorize">기억하세요...</span>';
  } else if (state.phase === 'recall') {
    var orderNote = config.orderMatters ? ' (순서 중요!)' : '';
    phaseEl.innerHTML = '<span class="mm-phase-recall">재현하세요!' + orderNote + ' &mdash; ' + state.selectedCells.length + '/' + state.pattern.length + ' 선택됨</span>';
  }
}
...
function updateActions(config) {
  ...
  actionsEl.innerHTML =
    '<button class="mm-btn mm-btn-outline" onclick="mmClearSelection()"' + clearDisabled + '>초기화</button>' +
    '<button class="mm-btn mm-btn-submit" onclick="mmSubmitAnswer()"' + submitDisabled + '>제출 (' + state.selectedCells.length + '/' + state.pattern.length + ')</button>';
}
...
function updateProgressBar(config) {
  ...
  document.getElementById('mm-progress-text').textContent =
    '라운드 ' + (state.roundsInLevel + 1) + ' / ' + config.roundsPerLevel;
}
...
function showFeedback(correct, config) {
  ...
  var phaseEl = document.getElementById('mm-phase');
  if (correct) {
    phaseEl.innerHTML = '<span class="mm-phase-correct">정답!</span>';
  } else {
    phaseEl.innerHTML = '<span class="mm-phase-wrong">아쉽네요...</span>';
  }
...
function endGame() {
  ...
  if (isNewBest) {
    ...
    bestBadge.innerHTML = 'uD83CuDF1F 대단해요!신기록!';
...
document.getElementById('mm-share-btn').addEventListener('click', function() {
  GK.shareResult(state.score, 'Memory Matrix', '/ko/games/memory-matrix/', 'mm-share-copied');
});
})();
</script>