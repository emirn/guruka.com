---
title: "Sequence Recall - Jogo de Treinamento Cerebral"
description: "Treine sua memoria de trabalho memorizando sequencias de cores. Observe as cores e lembre-as na ordem correta!"
full_width: true
language: "pt"
---
...
<p class="sr-subtitle">Treine sua memoria de trabalho</p>
<ul class="sr-instructions-list">
<li data-num="1">Observe a sequencia de cores mostrada uma a uma</li>
<li data-num="2">Memorize a ordem cuidadosamente</li>
<li data-num="3">Lembre as cores na mesma ordem exata</li>
<li data-num="4">Passe por 20 niveis com dificuldade crescente</li>
</ul>
<div class="sr-best-score" id="sr-best-display">
Recorde Pessoal: <strong id="sr-best-value">0</strong> Pontos
</div>
<button class="sr-btn-primary" id="sr-start-btn">Iniciar Jogo</button>
<div id="sr-challenge-banner-wrap"></div>
<input type="text" id="sr-player-name" class="gk-name-input" placeholder="Seu nome (para compartilhar)" maxlength="20">
</div>
</div>
...
<span class="sr-header-label">Pontos</span>
...
<div class="sr-header-item">
<span class="sr-header-label">Sequencia</span>
...
<button class="gk-finish-btn" id="sr-finish-btn">Encerrar Jogo</button>
</div>

<div id="sr-complete" class="sr-screen">
<div class="sr-card sr-complete">
<div class="sr-complete-icon">&#x1F3C6;</div>
<h2 class="sr-complete-title">Jogo Concluido!</h2>
<div id="sr-new-best-badge" class="sr-new-best" style="display:none;">Novo Recorde Pessoal!</div>
...
<span class="sr-stat-label">Pontos</span>
</div>
<div class="sr-stat-box">
...
<span class="sr-stat-label">Precisao</span>
</div>
</div>
<div class="sr-complete-links">
<button class="sr-btn-primary" id="sr-play-again-btn">Jogar Novamente</button>
<a href="/pt/games/" class="sr-btn-secondary">Todos os Jogos</a>
</div>
<div class="gk-share-section">
<div class="gk-share-title">Desafie um amigo</div>
<button class="gk-share-btn" id="sr-share-btn">&#128279; Compartilhar resultado</button>
<div class="gk-share-copied" id="sr-share-copied" style="display:none;">Link copiado!</div>
</div>
</div>
</div>
...
var COLORS = [
{ name: 'Vermelho', hsl: 'hsl(0,84%,60%)' },
{ name: 'Azul', hsl: 'hsl(217,91%,60%)' },
{ name: 'Grün', hsl: 'hsl(142,76%,36%)' },
{ name: 'Amarelo', hsl: 'hsl(48,96%,53%)' },
{ name: 'Roxo', hsl: 'hsl(280,68%,55%)' },
{ name: 'Laranja', hsl: 'hsl(25,95%,53%)' }
];
...
$('sr-progress-label').textContent = 'Rodada ' + (state.round + 1) + ' de ' + ROUNDS_PER_LEVEL;
var pct = (state.round / ROUNDS_PER_LEVEL) * 100;
$('sr-progress-fill').style.width = pct + '%';
}

var SR_WIZARD_STEPS = [
{icon: '\uD83C\uDFA8', title: 'Observe a sequencia de cores', desc: 'As cores aparecem uma a uma. Memorize a ordem!'},
{icon: '\uD83E\uDDE0', title: 'Lembre-as em ordem', desc: 'Toque nas cores na ordem exata em que voce as viu.'},
{icon: '\uD83D\uDE80', title: 'Pronto? Vamos la!', desc: 'As sequencias ficam mais longas. Mantenha o foco!', final: true}
];
var srWizardStep = 0;
...
var btnLabel = s.final ? 'Comecar a Jogar' : 'Continuar';
...
$('sr-phase-label').textContent = 'Memorize as cores';
state.currentShowIndex = 0;
showNextColor();
}
...
$('sr-phase-label').textContent = 'Memorize as posicoes';
state.currentShowIndex = 0;
showNextPosition();
}
...
$('sr-phase-label').textContent = 'Calculo rapido!';
...
$('sr-phase-label').textContent = 'Lembre a sequencia de cores';
state.selectedColors = [];
...
html += '<button class="sr-undo-btn" id="sr-undo-btn">Desfazer</button>';
html += '<button class="sr-submit-btn" id="sr-submit-colors-btn"' + (state.selectedColors.length < needed ? ' disabled' : '') + '>Confirmar</button>';
html += '</div>';
...
$('sr-phase-label').textContent = 'Lembre as posicoes na ordem';
state.selectedPositions = [];
state.phaseStartTime = Date.now();
...
html += '<button class="sr-undo-btn" id="sr-undo-pos-btn">Desfazer</button>';
html += '<button class="sr-submit-btn" id="sr-submit-pos-btn"' + (state.selectedPositions.length < needed ? ' disabled' : '') + '>Confirmar</button>';
html += '</div>';
...
html += '<div class="sr-feedback-points">+' + points + ' Pontos</div>';
} else {
...
srShowConfetti();
iconEl.textContent = '\uD83C\uDF89';
titleEl.textContent = 'FANTASTICO!';
badge.className = 'sr-new-best-enhanced';
badge.innerHTML = '\uD83C\uDF1F FANTASTICO! Novo Recorde!';
...
GK.shareResult(state.score, 'Sequence Recall', '/pt/games/sequence-recall/', 'sr-share-copied');
});
