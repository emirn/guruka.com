---
title: "Memory Matrix - Jogo de Treinamento Cerebral"
description: "Fortaleca sua memoria espacial memorizando e recordando padroes em grade. Observe o padrao e toque nas celulas de memoria!"
full_width: true
language: "pt"
---
...
<p class="mm-subtitle">Teste sua memoria espacial</p>
<div class="mm-how-to">
<h3>Como Jogar:</h3>
<ol>
<li><span class="mm-step-num">1</span><span>Observe o padrao -- celulas acendem uma a uma</span></li>
<li><span class="mm-step-num">2</span><span>Memorize quais celulas acendem</span></li>
<li><span class="mm-step-num">3</span><span>Toque nas mesmas celulas de memoria</span></li>
<li><span class="mm-step-num">4</span><span>A partir do nivel 8, a ordem importa! Toque na ordem correta</span></li>
</ol>
</div>
<div id="mm-instr-best" class="mm-best-banner" style="display:none;"></div>
<button class="mm-btn mm-btn-primary" onclick="mmShowWizard()">Iniciar Jogo</button>
<div id="mm-challenge-banner-wrap"></div>
<input type="text" id="mm-player-name" class="gk-name-input" placeholder="Seu nome (para compartilhar)" maxlength="20">
</div>
</div>
...
<div class="mm-header-label">Pontos</div>
...
<div class="mm-header-item">
<div class="mm-header-label">Sequencia</div>
...
<div class="mm-progress-text" id="mm-progress-text">Runde 1 von 3</div>
</div>
<div class="mm-actions" id="mm-actions"></div>
<button class="gk-finish-btn" id="mm-finish-btn">Encerrar Jogo</button>
</div>
...
<h1 class="mm-title">Jogo Concluido!</h1>
<div class="mm-score-display" id="mm-final-score">0</div>
<div id="mm-new-best-badge" class="mm-new-best" style="display:none;">Novo Recorde Pessoal!</div>
...
<div class="mm-stat-label">Precisao</div>
</div>
<div class="mm-stat-card">
<div class="mm-stat-value" id="mm-final-streak">0</div>
<div class="mm-stat-label">Melhor Sequencia</div>
</div>
</div>
<div class="mm-complete-actions">
<button class="mm-btn mm-btn-primary" onclick="mmPlayAgain()">Jogar Novamente</button>
<a href="/pt/games/" class="mm-back-link">Voltar para todos os jogos</a>
</div>
<div class="gk-share-section">
<div class="gk-share-title">Desafie um amigo</div>
<button class="gk-share-btn" id="mm-share-btn">&#128279; Compartilhar resultado</button>
<div class="gk-share-copied" id="mm-share-copied" style="display:none;">Link copiado!</div>
</div>
</div>
</div>
...
function updateInstructionsBest() {
  ...
  if (best > 0 && banner) {
    ...
    banner.innerHTML = 'Recorde Pessoal: <strong>' + best.toLocaleString() + '</strong> Pontos';
  }
}

/* ---- Wizard ---- */
var MM_WIZARD_STEPS = [
{icon: '\uD83D\uDCA1', title: 'Observe as celulas que acendem', desc: 'As celulas acendem uma a uma. Preste atencao!'},
{icon: '\uD83D\uDC46', title: 'Toque nelas de memoria', desc: 'Recrie o padrao depois que ele desaparecer.'},
{icon: '\uD83D\uDE80', title: 'Pronto? Vamos la!', desc: 'Os padroes ficam mais dificeis conforme seu nivel sobe!', final: true}
];
var mmWizardStep = 0;
...
var btnLabel = s.final ? 'Jogar Agora' : 'Continuar';
...
function updatePhase(config) {
  ...
  if (state.phase === 'showing') {
    phaseEl.innerHTML = '<span class="mm-phase-memorize">Merk dir das Muster...</span>';
  } else if (state.phase === 'recall') {
    var orderNote = config.orderMatters ? ' (Ordem importa!)' : '';
    phaseEl.innerHTML = '<span class="mm-phase-recall">Du bist dran!' + orderNote + ' &mdash; ' + state.selectedCells.length + '/' + state.pattern.length + ' selecionadas</span>';
  }
}
...
function updateActions(config) {
  ...
  actionsEl.innerHTML =
    '<button class="mm-btn mm-btn-outline" onclick="mmClearSelection()"' + clearDisabled + '>Limpar</button>' +
...
function updateProgressBar(config) {
  ...
  document.getElementById('mm-progress-text').textContent =
    'Rodada ' + (state.roundsInLevel + 1) + ' de ' + config.roundsPerLevel;
}
...
function showFeedback(correct, config) {
  ...
  var phaseEl = document.getElementById('mm-phase');
  if (correct) {
    phaseEl.innerHTML = '<span class="mm-phase-correct">Richtig!</span>';
  } else {
    phaseEl.innerHTML = '<span class="mm-phase-wrong">Leider falsch...</span>';
  }
...
function endGame() {
  ...
  if (isNewBest) {
    ...
    bestBadge.innerHTML = '\uD83C\uDF1F FANTASTICO! Novo Recorde!';
...
document.getElementById('mm-share-btn').addEventListener('click', function() {
  GK.shareResult(state.score, 'Memory Matrix', '/pt/games/memory-matrix/', 'mm-share-copied');
});
})();
</script>
