---
title: "Sequence Recall - Juego de Entrenamiento Cerebral"
description: "Entrena tu memoria de trabajo memorizando secuencias de colores. ¡Observa los colores y luego recuérdalos en orden!"
full_width: true
language: "es"
---
<style>
...
<p class="sr-subtitle">Entrena tu memoria de trabajo</p>
<div class="sr-best-score" id="sr-best-display">
Mejor Puntuación: <strong id="sr-best-value">ninguno aún</strong>
</div>
<div class="sr-how-to">
<h3>Cómo Jugar</h3>
<ul>
<li>Observa las fichas de colores que se iluminan en secuencia.</li>
<li>Toca las fichas en el mismo orden de memoria.</li>
<li>En niveles más altos, resuelve una <strong>distracción matemática</strong> antes de recordar.</li>
<li>¡Desde el nivel 11, recuerda tanto <strong>colores como posiciones</strong> (doble pista)!</li>
</ul>
</div>

<button class="sr-btn-primary" id="sr-start-btn">Iniciar Juego</button>
...
<span class="sr-header-label">Puntuación</span>
<span class="sr-header-value" id="sr-score">0</span>
</div>
<div class="sr-header-item">
<span class="sr-header-label">Nivel</span>
<span class="sr-header-value" id="sr-level">1</span>
</div>
<div class="sr-header-item">
<span class="sr-header-label">Racha</span>
<span class="sr-header-value sr-streak" id="sr-streak">0</span>
</div>
</div>
<div class="sr-phase-label" id="sr-phase-label">Memoriza</div>
<div id="sr-play-area"></div>
<div class="sr-progress">
<div class="sr-progress-label" id="sr-progress-label">Ronda 1 de 3</div>
<div class="sr-progress-bar">
<div class="sr-progress-fill" id="sr-progress-fill" style="width:0%"></div>
</div>
</div>
<button class="gk-finish-btn" id="sr-finish-btn">Terminar Juego</button>
</div>

<div id="sr-complete" class="sr-screen">
<div class="sr-card sr-complete">
<div class="sr-complete-icon">&#x1F3C6;</div>
<h2 class="sr-complete-title">¡Juego Completado!</h2>
<div id="sr-new-best-badge" class="sr-new-best" style="display:none;">¡Nueva Mejor Puntuación!</div>
...
<span class="sr-stat-label">Puntuación</span>
</div>
<div class="sr-stat-box">
<span class="sr-stat-value" id="sr-final-level">1</span>
<span class="sr-stat-label">Nivel</span>
</div>
<div class="sr-stat-box">
<span class="sr-stat-value" id="sr-final-accuracy">0%</span>
<span class="sr-stat-label">Precisión</span>
</div>
</div>
<div class="sr-complete-links">
<button class="sr-btn-primary" id="sr-play-again-btn">Jugar de Nuevo</button>
<a href="/es/games/" class="sr-btn-secondary">Todos los Juegos</a>
</div>
<div class="gk-share-section">

<button class="gk-share-btn" id="sr-share-btn">&#128279; Comparte tu Puntuación</button>
<div class="gk-share-copied" id="sr-share-copied" style="display:none;">¡Enlace copiado!</div>
</div>
</div>
</div>
...
var COLORS = [
{ name: 'Rojo', hsl: 'hsl(0,84%,60%)' },
{ name: 'Azul', hsl: 'hsl(217,91%,60%)' },
{ name: 'Verde', hsl: 'hsl(142,76%,36%)' },
{ name: 'Amarillo', hsl: 'hsl(48,96%,53%)' },
{ name: 'Morado', hsl: 'hsl(280,68%,55%)' },
{ name: 'Naranja', hsl: 'hsl(25,95%,53%)' }
];
...
$('sr-progress-label').textContent = 'Ronda ' + (state.round + 1) + ' de ' + ROUNDS_PER_LEVEL;
var pct = (state.round / ROUNDS_PER_LEVEL) * 100;
$('sr-progress-fill').style.width = pct + '%';
}

var SR_WIZARD_STEPS = [
{icon: '\uD83C\uDFA8', title: 'Observa la secuencia de colores', desc: 'Los colores aparecen uno a la vez. ¡Memoriza el orden!'},
{icon: '\uD83E\uDDE0', title: 'Recuérdalos en orden', desc: 'Toca los colores en la secuencia exacta que viste.'},
{icon: '\uD83D\uDE80', title: '¿Listo? ¡Vamos!', desc: '¡Las secuencias se hacen más largas. Mantén la concentración!', final: true}
];
var srWizardStep = 0;
...
$('sr-phase-label').textContent = 'Memoriza los colores';
state.currentShowIndex = 0;
showNextColor();
}
...
$('sr-phase-label').textContent = 'Memoriza las posiciones';
state.currentShowIndex = 0;
showNextPosition();
}
...
$('sr-phase-label').textContent = '¡Matemáticas rápidas!';
...
html += '<button class="sr-math-submit" id="sr-math-btn">Ir</button>';
html += '</div>';
html += '<div class="sr-math-hint">Resuelve para continuar</div>';
html += '</div>';
...
$('sr-phase-label').textContent = 'Recuerda la secuencia de colores';
state.selectedColors = [];
...
html += '<span class="sr-selected-placeholder">Toca los colores en orden (' + needed + ' necesarios)</span>';
} else {
...
html += '<button class="sr-undo-btn" id="sr-undo-btn">Deshacer</button>';
html += '<button class="sr-submit-btn" id="sr-submit-colors-btn"' + (state.selectedColors.length < needed ? ' disabled' : '') + '>Enviar</button>';
html += '</div>';
...
$('sr-phase-label').textContent = 'Recuerda las posiciones en orden';
state.selectedPositions = [];
state.phaseStartTime = Date.now();
...
html += '<span class="sr-selected-placeholder">Toca las posiciones en orden (' + needed + ' necesarias)</span>';
} else {
...
html += '<button class="sr-undo-btn" id="sr-undo-pos-btn">Deshacer</button>';
html += '<button class="sr-submit-btn" id="sr-submit-pos-btn"' + (state.selectedPositions.length < needed ? ' disabled' : '') + '>Enviar</button>';
html += '</div>';
...
if (correct) {
html += '<div class="sr-feedback-correct">¡Correcto!</div>';
html += '<div class="sr-feedback-points">+' + points + ' puntos</div>';
} else {
html += '<div class="sr-feedback-wrong">Incorrecto</div>';
html += '<div class="sr-feedback-points">Racha reiniciada</div>';
}
html += '</div></div>';
...
badge.innerHTML = '\uD83C\uDF1F ¡INCREÍBLE! ¡Nuevo Récord!';
...
var best = getBestScore();
if (best > 0) {
$('sr-best-value').textContent = best.toLocaleString() + ' puntos';
} else {
$('sr-best-value').textContent = 'ninguno aún';
}
...
GK.shareResult(state.score, 'Sequence Recall', '/es/games/sequence-recall/', 'sr-share-copied');
});
