---
title: "Memory Matrix - Jeu d'Entraînement Cérébral"
description: "Renforcez votre mémoire spatiale en mémorisant et en rappelant des motifs de grille. Regardez le motif, puis tapez les cellules de mémoire !"
full_width: true
language: "fr"
---
...
<p class="mm-subtitle">Testez votre mémoire spatiale</p>
<div class="mm-how-to">
<h3>Comment Jouer :</h3>
<ol>
<li><span class="mm-step-num">1</span><span>Regardez le motif -- les cellules se mettent en surbrillance une par une</span></li>
<li><span class="mm-step-num">2</span><span>Mémorisez les cellules qui s'allument</span></li>
<li><span class="mm-step-num">3</span><span>Tapez les mêmes cellules de mémoire</span></li>
<li><span class="mm-step-num">4</span><span>À partir du niveau 8, l'ordre compte ! Tapez dans la bonne séquence</span></li>
</ol>
</div>
<div id="mm-instr-best" class="mm-best-banner" style="display:none;"></div>
<button class="mm-btn mm-btn-primary" onclick="mmShowWizard()">Commencer</button>
<div id="mm-challenge-banner-wrap"></div>
<input type="text" id="mm-player-name" class="gk-name-input" placeholder="Votre nom (pour partager)" maxlength="20">
</div>
</div>
...
<div class="mm-header-label">Score</div>
<div class="mm-header-value" id="mm-score">0</div>
</div>
<div class="mm-header-item">
<div class="mm-header-label">Niveau</div>
<div class="mm-header-value" id="mm-level">1</div>
</div>
<div class="mm-header-item">
<div class="mm-header-label">Série</div>
...
<div class="mm-progress-text" id="mm-progress-text">Manche 1 sur 3</div>
</div>
<div class="mm-actions" id="mm-actions"></div>
<button class="gk-finish-btn" id="mm-finish-btn">Terminer</button>
</div>
...
<h1 class="mm-title">Partie Terminée !</h1>
<div class="mm-score-display" id="mm-final-score">0</div>
<div id="mm-new-best-badge" class="mm-new-best" style="display:none;">Nouveau Record !</div>
...
<div class="mm-stat-label">Niveau</div>
</div>
<div class="mm-stat-card">
<div class="mm-stat-value" id="mm-final-accuracy">0%</div>
<div class="mm-stat-label">Précision</div>
</div>
<div class="mm-stat-card">
<div class="mm-stat-value" id="mm-final-streak">0</div>
<div class="mm-stat-label">Meilleure Série</div>
</div>
</div>
<div class="mm-complete-actions">
<button class="mm-btn mm-btn-primary" onclick="mmPlayAgain()">Rejouer</button>
<a href="/fr/games/" class="mm-back-link">Tous les Jeux</a>
</div>
<div class="gk-share-section">
<div class="gk-share-title">Défier un Ami</div>
<button class="gk-share-btn" id="mm-share-btn">&#128279; Partager votre Score</button>
<div class="gk-share-copied" id="mm-share-copied" style="display:none;">Lien copié !</div>
</div>
</div>
</div>
...
function updateInstructionsBest() {
  ...
  if (best > 0 && banner) {
    ...
    banner.innerHTML = 'Meilleur Score : <strong>' + best.toLocaleString() + '</strong>';
  }
}

/* ---- Wizard ---- */
var MM_WIZARD_STEPS = [
{icon: '\uD83D\uDCA1', title: 'Regardez les cellules s\'allumer', desc: 'Les cellules se mettent en surbrillance une par une. Attention !'},
{icon: '\uD83D\uDC46', title: 'Tapez-les de mémoire', desc: 'Recréez le motif après sa disparition.'},
{icon: '\uD83D\uDE80', title: 'Prêt ? C\'est parti !', desc: 'Les motifs deviennent plus difficiles à mesure que vous progressez !', final: true}
];
var mmWizardStep = 0;
...
function updatePhase(config) {
  ...
  if (state.phase === 'showing') {
    phaseEl.innerHTML = '<span class="mm-phase-memorize">Mémorisez le motif...</span>';
  } else if (state.phase === 'recall') {
    var orderNote = config.orderMatters ? ' (l\'ordre compte !)' : '';
    phaseEl.innerHTML = '<span class="mm-phase-recall">Tapez les cellules !' + orderNote + ' &mdash; ' + state.selectedCells.length + '/' + state.pattern.length + ' sélectionnées</span>';
  }
}
...
function updateActions(config) {
  ...
  actionsEl.innerHTML =
    '<button class="mm-btn mm-btn-outline" onclick="mmClearSelection()"' + clearDisabled + '>Effacer</button>' +
    '<button class="mm-btn mm-btn-submit" onclick="mmSubmitAnswer()"' + submitDisabled + '>Valider (' + state.selectedCells.length + '/' + state.pattern.length + ')</button>';
}
...
function updateProgressBar(config) {
  ...
  document.getElementById('mm-progress-text').textContent =
    'Manche ' + (state.roundsInLevel + 1) + ' sur ' + config.roundsPerLevel;
}
...
function showFeedback(correct, config) {
  ...
  var phaseEl = document.getElementById('mm-phase');
  if (correct) {
    phaseEl.innerHTML = '<span class="mm-phase-correct">Correct !</span>';
  } else {
    phaseEl.innerHTML = '<span class="mm-phase-wrong">Incorrect</span>';
  }
...
function endGame() {
  ...
  if (isNewBest) {
    ...
    bestBadge.innerHTML = '\uD83C\uDF1F INCROYABLE ! Nouveau Record !';
...
document.getElementById('mm-share-btn').addEventListener('click', function() {
  GK.shareResult(state.score, 'Memory Matrix', '/fr/games/memory-matrix/', 'mm-share-copied');
});
})();
</script>
