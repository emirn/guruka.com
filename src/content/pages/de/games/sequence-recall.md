---
title: "Sequence Recall - Gehirntraining Spiel"
description: "Trainiere dein Arbeitsgedächtnis, indem du dir Farbfolgen merkst. Beobachte die Farben und erinnere sie dann in der richtigen Reihenfolge!"
full_width: true
language: "de"
---
...
<p class="sr-subtitle">Trainiere dein Arbeitsgedächtnis</p>
<div class="sr-best-score" id="sr-best-display">
Persönlicher Rekord: <strong id="sr-best-value">noch keiner</strong>
</div>
<div class="sr-how-to">
<h3>So wird gespielt</h3>
<ul>
<li>Beobachte die farbigen Kacheln, die nacheinander aufleuchten.</li>
<li>Tippe die Kacheln in derselben Reihenfolge aus dem Gedächtnis.</li>
<li>Auf höheren Leveln löse eine <strong>Mathe-Ablenkung</strong>, bevor du dich erinnerst.</li>
<li>Ab Level 11 erinnere dich an <strong>Farben und Positionen</strong> (Dual-Track)!</li>
</ul>
</div>

<button class="sr-btn-primary" id="sr-start-btn">Spiel starten</button>
...
<span class="sr-header-label">Punkte</span>
<span class="sr-header-value" id="sr-score">0</span>
</div>
<div class="sr-header-item">
<span class="sr-header-label">Level</span>
<span class="sr-header-value" id="sr-level">1</span>
</div>
<div class="sr-header-item">
<span class="sr-header-label">Serie</span>
...
<div class="sr-progress-label" id="sr-progress-label">Runde 1 von 3</div>
<div class="sr-progress-bar">
<div class="sr-progress-fill" id="sr-progress-fill" style="width:0%"></div>
</div>
</div>
<button class="gk-finish-btn" id="sr-finish-btn">Spiel beenden</button>
</div>

<div id="sr-complete" class="sr-screen">
<div class="sr-card sr-complete">
<div class="sr-complete-icon">&#x1F3C6;</div>
<h2 class="sr-complete-title">Spiel beendet!</h2>
<div id="sr-new-best-badge" class="sr-new-best" style="display:none;">Neuer persönlicher Rekord!</div>
...
<span class="sr-stat-label">Punkte</span>
</div>
<div class="sr-stat-box">
...
<span class="sr-stat-label">Genauigkeit</span>
</div>
</div>
<div class="sr-complete-links">
<button class="sr-btn-primary" id="sr-play-again-btn">Nochmal spielen</button>
<a href="/de/games/" class="sr-btn-secondary">Alle Spiele</a>
</div>
<div class="gk-share-section">

<button class="gk-share-btn" id="sr-share-btn">&#128279; Teile dein Ergebnis</button>
<div class="gk-share-copied" id="sr-share-copied" style="display:none;">Link kopiert!</div>
</div>
</div>
</div>
...
var COLORS = [
{ name: 'Rot', hsl: 'hsl(0,84%,60%)' },
{ name: 'Blau', hsl: 'hsl(217,91%,60%)' },
{ name: 'Grün', hsl: 'hsl(142,76%,36%)' },
{ name: 'Gelb', hsl: 'hsl(48,96%,53%)' },
{ name: 'Lila', hsl: 'hsl(280,68%,55%)' },
...
$('sr-progress-label').textContent = 'Runde ' + (state.round + 1) + ' von ' + ROUNDS_PER_LEVEL;
var pct = (state.round / ROUNDS_PER_LEVEL) * 100;
$('sr-progress-fill').style.width = pct + '%';
}

var SR_WIZARD_STEPS = [
{icon: '\uD83C\uDFA8', title: 'Beobachte die Farbfolge', desc: 'Farben erscheinen nacheinander. Merke dir die Reihenfolge!'},
{icon: '\uD83E\uDDE0', title: 'Erinnere sie in Reihenfolge', desc: 'Tippe die Farben in exakt der Reihenfolge, die du gesehen hast.'},
{icon: '\uD83D\uDE80', title: 'Bereit? Los geht\'s!', desc: 'Die Folgen werden länger. Bleib fokussiert!', final: true}
];
var srWizardStep = 0;
...
var btnLabel = s.final ? 'Spielen beginnen' : 'Weiter';
...
$('sr-phase-label').textContent = 'Merke dir die Farben';
state.currentShowIndex = 0;
showNextColor();
}
...
$('sr-phase-label').textContent = 'Merke dir die Positionen';
state.currentShowIndex = 0;
showNextPosition();
}
...
$('sr-phase-label').textContent = 'Schnelles Rechnen!';
...
html += '<button class="sr-math-submit" id="sr-math-btn">Los</button>';
html += '</div>';
html += '<div class="sr-math-hint">Löse zum Fortfahren</div>';
html += '</div>';
...
$('sr-phase-label').textContent = 'Erinnere die Farbfolge';
state.selectedColors = [];
...
html += '<span class="sr-selected-placeholder">Tippe Farben in Reihenfolge (' + needed + ' nötig)</span>';
} else {
...
html += '<button class="sr-undo-btn" id="sr-undo-btn">Rückgängig</button>';
html += '<button class="sr-submit-btn" id="sr-submit-colors-btn"' + (state.selectedColors.length < needed ? ' disabled' : '') + '>Bestätigen</button>';
html += '</div>';
...
$('sr-phase-label').textContent = 'Erinnere die Positionen in der Reihenfolge';
state.selectedPositions = [];
state.phaseStartTime = Date.now();
...
html += '<span class="sr-selected-placeholder">Tippe Gitterpositionen in Reihenfolge (' + needed + ' nötig)</span>';
} else {
...
html += '<button class="sr-undo-btn" id="sr-undo-pos-btn">Rückgängig</button>';
html += '<button class="sr-submit-btn" id="sr-submit-pos-btn"' + (state.selectedPositions.length < needed ? ' disabled' : '') + '>Bestätigen</button>';
html += '</div>';
...
if (correct) {
html += '<div class="sr-feedback-correct">Richtig!</div>';
html += '<div class="sr-feedback-points">+' + points + ' Punkte</div>';
} else {
html += '<div class="sr-feedback-wrong">Falsch</div>';
html += '<div class="sr-feedback-points">Serie zurückgesetzt</div>';
}
html += '</div></div>';
...
srShowConfetti();
iconEl.textContent = '\uD83C\uDF89';
titleEl.textContent = 'FANTASTISCH!';
badge.className = 'sr-new-best-enhanced';
badge.innerHTML = '\uD83C\uDF1F FANTASTISCH! Neuer Rekord!';
...
var best = getBestScore();
if (best > 0) {
$('sr-best-value').textContent = best.toLocaleString() + ' Punkte';
} else {
$('sr-best-value').textContent = 'noch keiner';
}
...
GK.shareResult(state.score, 'Sequence Recall', '/de/games/sequence-recall/', 'sr-share-copied');
});
