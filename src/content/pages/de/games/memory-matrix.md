---
title: "Memory Matrix - Gehirntraining-Spiel"
description: "Stärke dein räumliches Gedächtnis, indem du Gittermuster auswendig lernst und abrufst. Beobachte das Muster und tippe dann die Zellen aus dem Gedächtnis!"
full_width: true
language: "de"
---
...
<input type="text" id="mm-player-name" class="gk-name-input" placeholder="Dein Name (zum Teilen)" maxlength="20">
</div>
</div>
...
<div class="mm-header-label">Punkte</div>
...
<div class="mm-header-item">
<div class="mm-header-label">Serie</div>
...
<button class="gk-finish-btn" id="mm-finish-btn">Spiel beenden</button>
</div>
...
<div class="mm-stat-label">Genauigkeit</div>
</div>
<div class="mm-stat-card">
<div class="mm-stat-value" id="mm-final-streak">0</div>
<div class="mm-stat-label">Beste Serie</div>
</div>
</div>
<div class="mm-complete-actions">
<button class="mm-btn mm-btn-primary" onclick="mmPlayAgain()">Nochmal spielen</button>
<a href="/de/games/" class="mm-back-link">Zurück zu allen Spielen</a>
</div>
<div class="gk-share-section">
<div class="gk-share-title">Fordere einen Freund heraus</div>
<button class="gk-share-btn" id="mm-share-btn">&#128279; Teile dein Ergebnis</button>
<div class="gk-share-copied" id="mm-share-copied" style="display:none;">Link kopiert!</div>
</div>
</div>
</div>
...
/* ---- Wizard ---- */
var MM_WIZARD_STEPS = [
{icon: '\uD83D\uDCA1', title: 'Beobachte die aufleuchtenden Zellen', desc: 'Die Zellen leuchten nacheinander auf. Pass gut auf!'},
{icon: '\uD83D\uDC46', title: 'Tippe sie aus dem Gedächtnis an', desc: 'Erstelle das Muster nach, nachdem es verschwunden ist.'},
{icon: '\uD83D\uDE80', title: 'Bereit? Los geht\u2019s!', desc: 'Die Muster werden schwieriger, je höher dein Level!', final: true}
];
var mmWizardStep = 0;
...
var btnLabel = s.final ? 'Jetzt spielen' : 'Weiter';
...
function updatePhase(config) {
  ...
  if (state.phase === 'showing') {
    ...
    var orderNote = config.orderMatters ? ' (Reihenfolge wichtig!)' : '';
    phaseEl.innerHTML = '<span class="mm-phase-recall">Du bist dran!' + orderNote + ' &mdash; ' + state.selectedCells.length + '/' + state.pattern.length + ' ausgewählt</span>';
  }
}
...
function updateActions(config) {
  ...
  actionsEl.innerHTML =
    '<button class="mm-btn mm-btn-outline" onclick="mmClearSelection()"' + clearDisabled + '>Löschen</button>' +
    '<button class="mm-btn mm-btn-submit" onclick="mmSubmitAnswer()"' + submitDisabled + '>Abschicken (' + state.selectedCells.length + '/' + state.pattern.length + ')</button>';
}
...
function updateProgressBar(config) {
  ...
  document.getElementById('mm-progress-text').textContent =
    'Runde ' + (state.roundsInLevel + 1) + ' von ' + config.roundsPerLevel;
}
...
function endGame() {
  ...
  if (isNewBest) {
    ...
    bestBadge.innerHTML = '\uD83C\uDF1F FANTASTISCH! Neuer Rekord!';
...
document.getElementById('mm-share-btn').addEventListener('click', function() {
  GK.shareResult(state.score, 'Memory Matrix', '/de/games/memory-matrix/', 'mm-share-copied');
});
})();
</script>
