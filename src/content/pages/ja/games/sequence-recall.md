---
title: "Sequence Recall - 脳トレゲーム"
description: "色の順番を覚えてワーキングメモリを鍛えましょう。色を観察して、正しい順序で思い出してください！"
full_width: true
language: "ja"
---
...
<p class="sr-subtitle">ワーキングメモリを鍛えよう</p>
<div class="sr-best-score" id="sr-best-display">
自己ベスト: <strong id="sr-best-value">まだなし</strong>
</div>
<div class="sr-how-to">
<h3>遊び方</h3>
<ul>
<li>色のついたタイルが順番に光るのを見てください。</li>
<li>記憶を頼りに同じ順番でタイルをタップしてください。</li>
<li>高レベルでは、思い出す前に<strong>計算問題</strong>を解きます。</li>
<li>レベル11からは<strong>色と位置の両方</strong>を覚えましょう（デュアルトラック）！</li>
</ul>
</div>

<button class="sr-btn-primary" id="sr-start-btn">ゲームスタート</button>
...
<span class="sr-header-label">スコア</span>
...
<div class="sr-header-item">
<span class="sr-header-label">連続</span>
...
<div class="sr-progress-label" id="sr-progress-label">ラウンド 1 / 3</div>
<div class="sr-progress-bar">
<div class="sr-progress-fill" id="sr-progress-fill" style="width:0%"></div>
</div>
</div>
<button class="gk-finish-btn" id="sr-finish-btn">ゲーム終了</button>
</div>

<div id="sr-complete" class="sr-screen">
<div class="sr-card sr-complete">
<div class="sr-complete-icon">&#x1F3C6;</div>
<h2 class="sr-complete-title">ゲーム終了！</h2>
<div id="sr-new-best-badge" class="sr-new-best" style="display:none;">新しい自己ベスト！</div>
...
<span class="sr-stat-label">スコア</span>
</div>
<div class="sr-stat-box">
...
<span class="sr-stat-label">正確性</span>
</div>
</div>
<div class="sr-complete-links">
<button class="sr-btn-primary" id="sr-play-again-btn">もう一度プレイ</button>
<a href="/ja/games/" class="sr-btn-secondary">すべてのゲーム</a>
</div>
<div class="gk-share-section">

<button class="gk-share-btn" id="sr-share-btn">&#128279; 結果をシェア</button>
<div class="gk-share-copied" id="sr-share-copied" style="display:none;">リンクをコピーしました！</div>
</div>
</div>
</div>
...
var COLORS = [
{ name: '赤', hsl: 'hsl(0,84%,60%)' },
{ name: '青', hsl: 'hsl(217,91%,60%)' },
{ name: '緑', hsl: 'hsl(142,76%,36%)' },
{ name: '黄', hsl: 'hsl(48,96%,53%)' },
{ name: '紫', hsl: 'hsl(280,68%,55%)' },
{ name: 'オレンジ', hsl: 'hsl(25,95%,53%)' }
];
...
$('sr-progress-label').textContent = 'ラウンド ' + (state.round + 1) + ' / ' + ROUNDS_PER_LEVEL;
var pct = (state.round / ROUNDS_PER_LEVEL) * 100;
$('sr-progress-fill').style.width = pct + '%';
}

var SR_WIZARD_STEPS = [
{icon: '\uD83C\uDFA8', title: '色の並びを観察しよう', desc: '色が順番に表示されます。順番を覚えてください！'},
{icon: '\uD83E\uDDE0', title: '順番通りに思い出そう', desc: '見た順番通りに色をタップしてください。'},
{icon: '\uD83D\uDE80', title: '準備はいい？スタート\'s!', desc: '並びは長くなります。集中してください！', final: true}
];
var srWizardStep = 0;
...
$('sr-phase-label').textContent = '色を覚えてください';
state.currentShowIndex = 0;
showNextColor();
}
...
$('sr-phase-label').textContent = '位置を覚えてください';
state.currentShowIndex = 0;
showNextPosition();
}
...
$('sr-phase-label').textContent = '計算チャレンジ！';
...
html += '</div>';
html += '<div class="sr-math-hint">解いて進む</div>';
html += '</div>';
...
$('sr-phase-label').textContent = '色の並びを思い出してください';
state.selectedColors = [];
...
html += '<span class="sr-selected-placeholder">色を順番にタップ（' + needed + ' 個必要）</span>';
} else {
...
html += '<button class="sr-undo-btn" id="sr-undo-btn">元に戻す</button>';
html += '<button class="sr-submit-btn" id="sr-submit-colors-btn"' + (state.selectedColors.length < needed ? ' disabled' : '') + '>確定</button>';
html += '</div>';
...
$('sr-phase-label').textContent = '順番通りに位置を思い出してください';
state.selectedPositions = [];
state.phaseStartTime = Date.now();
...
html += '<span class="sr-selected-placeholder">グリッド位置を順番にタップ（' + needed + ' 個必要）</span>';
} else {
...
html += '<button class="sr-undo-btn" id="sr-undo-pos-btn">元に戻す</button>';
html += '<button class="sr-submit-btn" id="sr-submit-pos-btn"' + (state.selectedPositions.length < needed ? ' disabled' : '') + '>確定</button>';
html += '</div>';
...
if (correct) {
html += '<div class="sr-feedback-correct">正解！</div>';
html += '<div class="sr-feedback-points">+' + points + ' ポイント</div>';
} else {
html += '<div class="sr-feedback-wrong">不正解</div>';
...
srShowConfetti();
iconEl.textContent = '\uD83C\uDF89';
titleEl.textContent = 'すばらしい！';
badge.className = 'sr-new-best-enhanced';
badge.innerHTML = 'uD83CuDF1F すばらしい！新記録！';
badge.style.display = 'inline-block';
} else {
iconEl.textContent = '\uD83C\uDFC6';
titleEl.textContent = 'お疲れさまでした！';
...
var best = getBestScore();
if (best > 0) {
$('sr-best-value').textContent = best.toLocaleString() + ' ポイント';
} else {
$('sr-best-value').textContent = 'まだなし';
}
...
GK.shareResult(state.score, 'Sequence Recall', '/ja/games/sequence-recall/', 'sr-share-copied');
});
