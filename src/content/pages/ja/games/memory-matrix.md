---
title: "Memory Matrix - 脳トレゲーム"
description: "グリッドパターンを覚えて再現する空間記憶トレーニング。パターンを観察して、記憶からセルをタップしてください！"
full_width: true
language: "ja"
---
...
<h3>遊び方:</h3>
<ol>
<li><span class="mm-step-num">1</span><span>パターンを観察 -- セルが順番に光ります</span></li>
<li><span class="mm-step-num">2</span><span>どのセルが光ったか覚えてください</span></li>
<li><span class="mm-step-num">3</span><span>記憶から同じセルをタップしてください</span></li>
<li><span class="mm-step-num">4</span><span>レベル8からは順番が重要です！正しい順番でタップしてください</span></li>
</ol>
</div>
...
<div class="mm-header-label">スコア</div>
...
<div class="mm-header-item">
<div class="mm-header-label">連続</div>
...
<div class="mm-progress-text" id="mm-progress-text">ラウンド 1 / 3</div>
</div>
<div class="mm-actions" id="mm-actions"></div>
<button class="gk-finish-btn" id="mm-finish-btn">ゲーム終了</button>
</div>
...
<h1 class="mm-title">ゲーム終了！</h1>
<div class="mm-score-display" id="mm-final-score">0</div>
<div id="mm-new-best-badge" class="mm-new-best" style="display:none;">新しい自己ベスト！</div>
...
<div class="mm-stat-label">正確性</div>
</div>
<div class="mm-stat-card">
<div class="mm-stat-value" id="mm-final-streak">0</div>
<div class="mm-stat-label">最高連続</div>
</div>
</div>
<div class="mm-complete-actions">
<button class="mm-btn mm-btn-primary" onclick="mmPlayAgain()">もう一度プレイ</button>
...
function updateInstructionsBest() {
  ...
  if (best > 0 && banner) {
    ...
    banner.innerHTML = '自己ベスト: <strong>' + best.toLocaleString() + '</strong> ポイント';
  }
}

/* ---- Wizard ---- */
var MM_WIZARD_STEPS = [
{icon: '\uD83D\uDCA1', title: '光るセルを観察しよう', desc: 'セルが順番に光ります。よく見てください！'},
{icon: '\uD83D\uDC46', title: '記憶からタップしよう', desc: 'パターンが消えた後、記憶から再現してください。'},
{icon: '\uD83D\uDE80', title: '準備はいい？スタート！', desc: 'レベルが上がるとパターンが難しくなります！', final: true}
];
var mmWizardStep = 0;
...
function updatePhase(config) {
  ...
  if (state.phase === 'showing') {
    phaseEl.innerHTML = '<span class="mm-phase-memorize">パターンを覚えて...</span>';
  } else if (state.phase === 'recall') {
    var orderNote = config.orderMatters ? ' (順番が重要！)' : '';
    phaseEl.innerHTML = '<span class="mm-phase-recall">あなたの番！' + orderNote + ' &mdash; ' + state.selectedCells.length + '/' + state.pattern.length + ' 選択済み</span>';
  }
}
...
function updateActions(config) {
  ...
  actionsEl.innerHTML =
    '<button class="mm-btn mm-btn-outline" onclick="mmClearSelection()"' + clearDisabled + '>クリア</button>' +
    '<button class="mm-btn mm-btn-submit" onclick="mmSubmitAnswer()"' + submitDisabled + '>送信 (' + state.selectedCells.length + '/' + state.pattern.length + ')</button>';
}
...
function updateProgressBar(config) {
  ...
  document.getElementById('mm-progress-text').textContent =
    'ラウンド ' + (state.roundsInLevel + 1) + ' / ' + config.roundsPerLevel;
}
...
function showFeedback(correct, config) {
  ...
  var phaseEl = document.getElementById('mm-phase');
  if (correct) {
    phaseEl.innerHTML = '<span class="mm-phase-correct">正解！</span>';
  } else {
    phaseEl.innerHTML = '<span class="mm-phase-wrong">残念...</span>';
  }
...
function endGame() {
  ...
  if (isNewBest) {
    ...
    bestBadge.innerHTML = 'uD83CuDF1F すばらしい！新記録！';
...
document.getElementById('mm-share-btn').addEventListener('click', function() {
  GK.shareResult(state.score, 'Memory Matrix', '/ja/games/memory-matrix/', 'mm-share-copied');
});
})();
</script>
