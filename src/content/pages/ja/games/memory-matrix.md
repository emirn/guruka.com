...
自己ベスト: <strong id="mm-best-value">まだなし</strong>
</div>
<div class="mm-how-to">
<h3>遊び方</h3>
<ul>
<li>パターンを観察 &mdash; セルが順番に光ります。</li>
<li>どのセルが光ったか覚えてください。</li>
<li>記憶から同じセルをタップしてください。</li>
<li>レベル8からは<strong>順番が重要</strong> &mdash; 正しい順番でタップしてください！</li>
</ul>
</div>

<button class="mm-btn mm-btn-primary" onclick="mmStartGame()">ゲームスタート</button>
...
function updateInstructionsBest() {
  ...
  if (el) {
    if (best > 0) {
      el.textContent = best.toLocaleString() + ' ポイント';
    } else {
      el.textContent = 'まだなし';
    }
  }
